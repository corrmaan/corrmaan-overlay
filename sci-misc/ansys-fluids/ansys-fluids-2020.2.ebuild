...
src_install() {
	...
	"${S}/INSTALL" -usetempdir "${T}/${MY_P}" -silent${myargs} -install_dir "${ED}/${INSTDIR}" || die
	use doc && "${S_DOC}/INSTALL" -usetempdir "${T}/${MY_P_DOC}" -silent -install_dir "${ED}/${INSTDIR}" || die
	...
	if use fluent; then
		...
		if use X; then
			...
			make_desktop_entry "${EPREFIX}/${INSTDIR}/${RELID}/fluent/bin/fluent" "ANSYS FLUENT v${RELYEAR}R${RELREV}" "Science;Physics"
