...
SRC_URI="https://ftp.lstc.com/anonymous/outgoing/${PN}/dev/linux64/${PN}-${MY_PV}-common_gtk3-${BUILDDATE}.tgz
	doc? ( https://ftp.lstc.com/anonymous/outgoing/${PN}/dev/doc/linux/Document.zip -> ${PN}-4.9-Document.zip
		https://ftp.lstc.com/anonymous/outgoing/${PN}/4.9/doc/linux/Tutor.zip -> ${PN}-4.9-Tutor.zip )"
...
src_unpack() {
	...
	unpack ${PN}-${MY_PV}-common_gtk3-${BUILDDATE}.tgz
...
src_prepare() {
	...
	cp "${DISTDIR}/${PN}-4.9-Document.zip" Document.zip
	cp "${DISTDIR}/${PN}-4.9-Tutor.zip" Tutor.zip
...
src_install() {
	...
	doins -r lib2
	insopts -m0644
	doins -r BaoSteel lspp_forming_49 lspp_matlib material.xml
