...
CONFIG_CHECK="~DRM ~DRM_KMS_HELPER ~USB_SUPPORT ~USB_ARCH_HAS_HCD"

pkg_setup() {
	linux-mod_pkg_setup
	kernel_is -ge 4 15 || die "Update your kernel to at least version 4.15."
	if kernel_is -gt 5 12; then
		ewarn "Kernel versions only up to 5.12 are officially supported."
	fi
}
...
src_compile() {
	KVER="${KV_FULL}" linux-mod_src_compile
